...
describe 'Quadlets::Unit::Unit' do
  ...
  %w[Wants Requires Requisite BindsTo PartOf Upholds Conflicts].each do |depend|
    context "with a key of #{depend} can have values of quadlets" do
      it { is_expected.to allow_value({ depend => 'centos.container' }) }
      it { is_expected.to allow_value({ depend => 'centos.volume' }) }
    end
  end
