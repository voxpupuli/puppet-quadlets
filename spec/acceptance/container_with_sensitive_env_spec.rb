...
describe 'quadlets::quadlet' do
  context 'with a simple CentOS container running with Sensitive Environment' do
      ...
      let(:manifest) do
        ...
        quadlets::quadlet{'centos.container':
          ...
          container_entry => {
            ...
            'Exec'   => 'sh -c "sleep inf"',
            'Environment' => [Sensitive("MYENV=password")],
          },
          install_entry   => {
    ...
    describe file('/etc/containers/systemd/centos.container') do
      it { is_expected.to be_file }
      it { is_expected.to be_owned_by 'root' }
      it { is_expected.to be_grouped_into 'root' }
      its(:content) { is_expected.to match %r{^Environment=MYENV=password$} }
    end
  end
end
