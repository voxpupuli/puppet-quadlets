    ...
    on_supported_os.each do |os, os_facts|
      context "on #{os}" do
        ...
        context 'with manage update timer enabled and timer enabled' do
          ...
          it { is_expected.to contain_service('podman-auto-update.timer').with_enable(true) }
        end

        context 'with manage update timer enabled, timer enabled, and a custom name' do
          let(:params) do
            { manage_autoupdate_timer: true, autoupdate_timer_enable: true, autoupdate_timer_name: 'test.unit' }
          end

          it { is_expected.to contain_service('test.unit').with_enable(true) }
        end
